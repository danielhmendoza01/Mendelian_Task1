...
```{r}
# Counting males
male_count <- sum(sub_UKBB$f.22001.0.0 == "Male")
# Counting females
female_count <- sum(sub_UKBB$f.22001.0.0 == "Female")
...
This made the columns binary.
Either they have never have had noises (such as ringing or buzzing) in your head or in one or both ears that lasts for more than five minutes at a time (represented by 0) or have had at some point (represente by 1).
...
```{r}
# Count TRUE in'cases' column
true_count <- sum(df_recode$cases)
false_count <- sum(df_recode$cases == FALSE)
# Print the count
cat("There are", true_count, "cases and", false_count, "controls.", "\n")
```
...
Their age corresponding to each hospital visit (1st to 4th time).

**Q7:** For those with more than one age records, which age do you think should be used in the genetic association analysis?
I think the fist visit age should be used because it will be consistant with the other participants and when the initial problem occured.
**Q8:** Please compute a summary of age information for controls (you can use `summary()` function in R):
```{r}
summary_age_0_0 <- summary(df_controls$f.21003.0.0)
print(summary_age_0_0)
```
\newpage
...
summary(fit_1)
```
Weight Negative Coefficient: -0.007647
Weight p value: <2e-16
There is a significant negative association between the mpg of the car and the weight of the car. As the weight of the car increases, its mpg decreases.
...
```{r, echo=FALSE}
# Create a new variable indicating if mpg is greater than 23
auto_data$mpg_gt_23 <- ifelse(auto_data$mpg > 23, 1, 0)

# Fit a logistic regression model
fit_logit <- glm(mpg_gt_23 ~ weight, data = auto_data, family = binomial)
...
weight -0.0040954
pvalue <2e-16
This model also shows that statistically significant negative association between the weight of the car and the likelihood of having an MPG greater than 23.
...
The estimates from the linear regression model indicate that for each additional unit increase in the weight of the car, 
the mpg decreases by approximately 0.007647 units.
The intercept of 46.216524 represnents the estimated mpg when weight is zero. 

In the logistic regression model, the estimates show that for each additional unit increase in weight, 
the log-odds of having an mpg greater than 23 decreases by 0.0040954.
The intercept of 11.4448009, representing the log-odds of having an mpg greater than 23 when weight is zero.

Both models indicate that as the weight of the car increases, its fuel efficiency decreases.
